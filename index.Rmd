...
# Chart 6 {data-navmenu="Page 3"}

Sidebar {.sidebar}
-------------------------------------  

The first track, Blow Out, although 
...
```{r}
littlebylittle <-
  get_tidy_audio_analysis("1R5qCDSE1XEiSDMkA0LAhf") |>
  select(segments) |>
  unnest(segments) |>
  select(start, duration, pitches)
...
littlebylittle |>
  ...
  geom_tile() +
  labs(x = "Time (s)", y = NULL, fill = "Magnitude", title="Little by Little") +
  theme_minimal() +
  scale_fill_viridis_c()
```
...
```{r}
littlebylittle <-
  get_tidy_audio_analysis("1R5qCDSE1XEiSDMkA0LAhf") |> # Change URI.
  compmus_align(bars, segments) |>                     # Change `bars`
  select(bars) |>                                      #   in all three
  unnest(bars) |>                                      #   of these lines.
  mutate(
